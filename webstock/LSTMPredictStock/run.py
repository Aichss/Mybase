...
from LSTMPredictStock.core import model
...
def train_model(stock_code, predict=True):  # 训练指定股票代码的模型
    '''
...
def prediction(stock_code, real=True, pre_len=30, plot=False):
    ...
    file_name=os.path.join(get_data_path(), stock_code + ".csv")
    dataframe = pd.read_csv(file_name)
    start_data=dataframe.iloc[-pre_len][0]
    ...
    return format_predictions(predictions,start_data)

def format_predictions(predictions,cur):    # 给预测数据添加对应日期
    date_predict = []
    #cur = datetime.now()
...
def predict_all_stock(pre_len=10):
    ...
    predict_list = []
    for stock_code in companies.keys():
        predict_list.append(prediction(stock_code=stock_code, real=False, pre_len=pre_len))
...
if __name__ == '__main__':
      #train_all_stock()
      #predict_all_stock()
      #train_model("000651", predict=True)
      print(prediction("000063"),False,30,False)